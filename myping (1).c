...
// took from https://www.geeksforgeeks.org/ping-in-c/
...
#define DESTINATION_IP "172.217.4.46"
...
{ 
	...
	(*addr_con).sin_family = AF_INET;//host_entity->h_addrtype; //must always be AF_INET The reason for this is so that it's possible to tell the actual type of a generalized socket descriptor.
...
{ 
	...
	// send icmp packet in a loop untile recive massage 
	while(pingloop) 
	{ 
		...
		{ 
			...
			{ 
				...
				{ 
					printf("from (h: %s) (%s) rtt = %Lf ms.\n", rev_host, ping_ip, rtt_msec); 
